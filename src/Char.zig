...
pub fn Char(comptime UserState: type) type {
    return struct {
        const Result = @import("Parser.zig").Parser(UserState).Result;

        pub fn symbol(stream: Stream, allocator: std.mem.Allocator, _: *UserState, c: u8) anyerror!Result(u8) {
            if (stream.isEOF()) {
        ...
        pub fn oneOf(stream: Stream, allocator: std.mem.Allocator, _: *UserState, x: []const u8) anyerror!Result(u8) {
            if (stream.isEOF()) {
                var error_msg = std.ArrayList(u8).init(allocator);
                var writer = error_msg.writer();
                try writer.print("{}: Unexpected EndOfStream", .{stream});
        ...
        pub fn noneOf(stream: Stream, allocator: std.mem.Allocator, _: *UserState, x: []const u8) anyerror!Result(u8) {
            if (stream.isEOF()) {
                var error_msg = std.ArrayList(u8).init(allocator);
                var writer = error_msg.writer();
                try writer.print("{}: Unexpected EndOfStream", .{stream});
        ...
        pub fn range(stream: Stream, allocator: std.mem.Allocator, _: *UserState, l: u8, h: u8) anyerror!Result(u8) {
        ...
        pub fn upper(stream: Stream, allocator: std.mem.Allocator, _: *UserState) anyerror!Result(u8) {
            if (stream.isEOF()) {
                var error_msg = std.ArrayList(u8).init(allocator);
                var writer = error_msg.writer();
                try writer.print("{}: Unexpected EndOfStream", .{stream});
        ...
        pub fn lower(stream: Stream, allocator: std.mem.Allocator, _: *UserState) anyerror!Result(u8) {
            if (stream.isEOF()) {
                var error_msg = std.ArrayList(u8).init(allocator);
                var writer = error_msg.writer();
                try writer.print("{}: Unexpected EndOfStream", .{stream});
        ...
        pub fn alpha(stream: Stream, allocator: std.mem.Allocator, _: *UserState) anyerror!Result(u8) {
            if (stream.isEOF()) {
                var error_msg = std.ArrayList(u8).init(allocator);
                var writer = error_msg.writer();
                try writer.print("{}: Unexpected EndOfStream", .{stream});
        ...
        pub fn alphaNum(stream: Stream, allocator: std.mem.Allocator, _: *UserState) anyerror!Result(u8) {
            if (stream.isEOF()) {
                var error_msg = std.ArrayList(u8).init(allocator);
                var writer = error_msg.writer();
                try writer.print("{}: Unexpected EndOfStream", .{stream});
        ...
        pub fn digit(stream: Stream, allocator: std.mem.Allocator, _: *UserState) anyerror!Result(u8) {
            if (stream.isEOF()) {
                var error_msg = std.ArrayList(u8).init(allocator);
                var writer = error_msg.writer();
                try writer.print("{}: Unexpected EndOfStream", .{stream});
        ...
        pub fn octDigit(stream: Stream, allocator: std.mem.Allocator, _: *UserState) anyerror!Result(u8) {
            if (stream.isEOF()) {
                var error_msg = std.ArrayList(u8).init(allocator);
                var writer = error_msg.writer();
                try writer.print("{}: Unexpected EndOfStream", .{stream});
        ...
        pub fn hexDigit(stream: Stream, allocator: std.mem.Allocator, _: *UserState) anyerror!Result(u8) {
            if (stream.isEOF()) {
                var error_msg = std.ArrayList(u8).init(allocator);
                var writer = error_msg.writer();
                try writer.print("{}: Unexpected EndOfStream", .{stream});
        ...
        pub fn satisfy(stream: Stream, allocator: std.mem.Allocator, _: *UserState, fnc: *const fn (u8) bool) anyerror!Result(u8) {
            if (stream.isEOF()) {
                var error_msg = std.ArrayList(u8).init(allocator);
                var writer = error_msg.writer();
                try writer.print("{}: Unexpected EndOfStream", .{stream});
        ...
        pub fn space(stream: Stream, allocator: std.mem.Allocator, _: *UserState) anyerror!Result(u8) {
            if (stream.isEOF()) {
                var error_msg = std.ArrayList(u8).init(allocator);
                var writer = error_msg.writer();
                try writer.print("{}: Unexpected EndOfStream", .{stream});
        ...
        pub fn string(stream: Stream, allocator: std.mem.Allocator, _: *UserState, str: []const u8) anyerror!Result([]const u8) {
            if (stream.isEOF()) {
                var error_msg = std.ArrayList(u8).init(allocator);
                var writer = error_msg.writer();
                try writer.print("{}: Unexpected EndOfStream", .{stream});
